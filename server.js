...
const inferMemoryType = (content) => {
  if (/不喜欢|讨厌|别再|不要叫|不要|害怕|担心|焦虑|压力/.test(content)) return '禁忌';
...
const summarizeMemory = (userMsg, replyText, roleName) => {
  ...
  if (!raw || raw.length < 4 || raw.length > 260) return null;
  if (/^(你好|在吗|嗯|哦|好|哈哈|谢谢|再见|晚安|早安|是的|对的|不是|没有|可以|好的|行|ok)[。！？!?.\s]*$/i.test(raw)) return null;
  const keywords = /(我叫|叫我|我是|我的|喜欢|不喜欢|讨厌|记住|以后|希望|习惯|生日|朋友|家人|学校|工作|职业|住在|来自|约定|我们|正在|剧情|线索|案件|设定|今天|昨天|明天|最近|感觉|觉得|心情|开心|难过|伤心|生气|害怕|担心|焦虑|压力|累|困|饿|想|需要|想要|买了|去了|看了|听了|玩了|遇到|发生|告诉|聊|经历|故事|梦想|目标|计划|打算|准备|开始|学习|考试|上班|下班|加班|旅行|搬家|聚会|约会|电影|音乐|游戏|读书|运动|健身|吃饭|睡觉|休息|生病|医院|天气|下雨|节日|过年|圣诞|纪念|庆祝|礼物|感动|幸福|孤独|想念|思念|怀念|记得|过去|以前|小时候|长大|变化|特别|重要|在乎)/;
  if (!keywords.test(raw) && raw.length < 15) return null;
  ...
  } else if (/^(我不喜欢|我讨厌|我讨厌|我反感|我不想)/.test(raw)) {
    const m = raw.match(/(?:不喜欢|讨厌|反感|不想)([^\s,，。.！!？?]+)/);
    content = m ? `用户不太喜欢${m[1]}，要避免提及。` : `用户表达了某种反感或不喜欢的事物。`;
