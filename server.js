...
const config = {
  ...
  rechargePackages: String(process.env.RECHARGE_PACKAGES || '1:10,6:60,18:200,30:360,50:600,68:900,98:1400,128:2000,198:3200,328:5500')
    .split(',')
    .map((item) => {
...
const saveDataNow = () => {
  try {
    const payload = {
      ...
      worldBooks: Object.fromEntries([...userWorldBooks.entries()].map(([uid, list]) => [uid, list.slice(-100)])),
...
const loadData = () => {
  try {
    ...
    if (payload.chatMoments) { for (const [k,v] of Object.entries(payload.chatMoments)) { if (Array.isArray(v)) userMoments.set(String(k).slice(0,80), v); } }
    if (payload.chatWallets) { for (const [k,v] of Object.entries(payload.chatWallets)) { if (v) userWallets.set(String(k).slice(0,80), v); } }
    if (payload.chatGroups) { for (const [k,v] of Object.entries(payload.chatGroups)) { if (Array.isArray(v)) userGroups.set(String(k).slice(0,80), v); } }
    if (payload.chatSettings) { for (const [k,v] of Object.entries(payload.chatSettings)) { if (v) chatSettings.set(String(k).slice(0,80), v); } }
    if (payload.rulePresets) { for (const [k,v] of Object.entries(payload.rulePresets)) { if (v) rulePresets.set(String(k).slice(0,80), v); } }
    ...
    if (payload.worldBooks && typeof payload.worldBooks === 'object') {
      for (const [uid, list] of Object.entries(payload.worldBooks)) {
        if (Array.isArray(list)) userWorldBooks.set(String(uid).slice(0, 80), list.slice(-100));
      }
    }
...
app.use((req, _res, next) => {
  ...
  if (sessionToken && sessions.has(sessionToken)) {
    ...
    const uid = String(req.headers['x-user-id'] || req.headers['X-User-Id'] || '').trim();
...
app.post('/api/auth/register', (req, res) => {
  ...
  if (!username || username.length < 2) return fail(res, 400, 5001, '用户名至少2个字符。');
  if (!password || password.length < 4) return fail(res, 400, 5002, '密码至少4个字符。');
  for (const acc of accounts.values()) {
    if (acc.username === username) return fail(res, 409, 5003, '用户名已存在。');
  }
  const accountId = randomUUID();
  const userId = 'user_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8);
  const account = { id: accountId, username, password, userId, createdAt: new Date().toISOString() };
  accounts.set(accountId, account);
  const token = randomUUID();
  sessions.set(token, { accountId, userId, createdAt: new Date().toISOString() });
  getUser(userId);
  saveData();
...
app.post('/api/auth/login', (req, res) => {
  const username = String(req.body?.username || '').trim();
  const password = String(req.body?.password || '');
  if (!username || !password) return fail(res, 400, 5004, '请输入用户名和密码。');
  let account = null;
  for (const acc of accounts.values()) {
    if (acc.username === username && acc.password === password) { account = acc; break; }
  }
  ...
  if (!account && username === '宛萦风' && password === '841026') {
    ...
    if (!accounts.has(adminAccountId)) {
      account = { id: adminAccountId, username: '宛萦风', password: '841026', userId: 'admin-user', createdAt: new Date().toISOString() };
  ...
  const token = randomUUID();
  sessions.set(token, { accountId: account.id, userId: account.userId, createdAt: new Date().toISOString() });
  getUser(account.userId);
  const isAdmin = account.username === '宛萦风';
  ok(res, { token, userId: account.userId, username: account.username, isAdmin }, '登录成功');
});

app.get('/api/auth/me', (req, res) => {
  const sessionToken = String(req.headers['x-session-token'] || '').trim();
  if (!sessionToken || !sessions.has(sessionToken)) return ok(res, { loggedIn: false });
  const session = sessions.get(sessionToken);
  const account = accounts.get(session.accountId);
  if (!account) return ok(res, { loggedIn: false });
  ok(res, { loggedIn: true, userId: account.userId, username: account.username, isAdmin: account.username === '宛萦风' });
});
...
function loadEmbedFiles() {
  ...
  const staticFiles = {
    ...
    'forum.js': 'application/javascript; charset=utf-8',
    'forum-avatars-base64.js': 'application/javascript; charset=utf-8',
    'novel-game.css': 'text/css; charset=utf-8',
    'novel-game.js': 'application/javascript; charset=utf-8',
  ...
  for (let i = 1; i <= 8; i++) {
    ...
    if (fs.existsSync(avatarPath)) {
      ...
      EMBED_PUBLIC['/' + 'avatars/avatar' + i + '.png'] = EMBED_PUBLIC['avatars/avatar' + i + '.png'];
    }
  }
}
loadEmbedFiles();
...
app.use('/js', express.static(path.join(__dirname, 'public', 'js'), { setHeaders: (res) => res.setHeader('Cache-Control', 'no-cache') }));
...
app.post('/api/voice/upload', (req, res) => {
  ...
  list.push({ id, blob: String(blob).slice(0, 5000000), duration: Math.min(Number(duration) || 0, 600), createdAt: Date.now() });
...
app.post('/api/moments', (req, res) => {
  ...
  const moment = {
    ...
    images: Array.isArray(images) ? images.slice(0, 9).map(i => String(i).slice(0, 500)) : [],
    likes: [],
    comments: [],
    createdAt: Date.now()
  };
...
app.post('/api/moments/:id/comment', (req, res) => {
  ...
  const m = list.find(x => x.id === req.params.id);
  if (!m) return fail(res, 404, 4041, '动态不存在');
  const c = { id: randomUUID(), userId: String(req.body?.userId || 'me').slice(0, 30), name: String(req.body?.name || '我').slice(0, 30), text: String(req.body?.text || '').slice(0, 200), createdAt: Date.now() };
  m.comments.push(c);
  ok(res, c, '评论成功');
});
...
app.post('/api/wallet/transfer', (req, res) => {
  ...
  w.transactions.push({ id: randomUUID(), amount: amt, direction, note: String(note || '').slice(0, 100), createdAt: Date.now() });
  if (w.transactions.length > 100) w.transactions.splice(0, w.transactions.length - 100);
  userWallets.set(uid, w);
  ok(res, w, '转账成功');
});
...
app.post('/api/groups', (req, res) => {
  ...
  const g = { id: randomUUID(), name: String(name).slice(0, 30), members: Array.isArray(members) ? members.slice(0, 50) : [], createdAt: Date.now() };
  list.push(g);
...
app.post('/api/redpacket/send', (req, res) => {
  ...
  w.balance = Math.round((w.balance - amt) * 100) / 100;
  userWallets.set(uid, w);
  ok(res, { redpacket: { id: randomUUID(), amount: amt, type: type === 'exclusive' ? 'exclusive' : 'lucky', note: String(note || '').slice(0, 50), groupId: String(groupId || '').slice(0, 80), claims: [], createdAt: Date.now() }, wallet: w }, '红包发送成功');
});
...
app.post('/api/heartvoices/:roleId', (req, res) => {
  ...
  const hv = { id: randomUUID(), text: String(req.body?.text || '').slice(0, 500), createdAt: Date.now() };
  list.unshift(hv);
  if (list.length > 100) list.splice(100);
...
app.post('/api/album', (req, res) => {
  ...
  const photo = { id: randomUUID(), url: String(url).slice(0, 500000), desc: String(desc || '').slice(0, 200), createdAt: Date.now() };
  list.unshift(photo);
...
app.get('/api/chat-settings', (req, res) => {
  const uid = String(req.userId).slice(0, 80);
  const s = chatSettings.get(uid) || { global: { timeAwareness: true, memoryCount: 20, summaryTrigger: 30, backgroundMessage: true }, perContact: {} };
...
const DEFAULT_THINKING_PRESETS = [
  { id: 'think_default', name: '默认思维链', content: '简短思考：分析用户情绪、决定回复策略，不超50字', enabled: true },
  { id: 'think_emotion', name: '情绪优先', content: '先判断用户当前情绪状态（开心/难过/焦虑/愤怒/平淡），再决定回复基调。若用户情绪低落，优先共情安慰而非给建议。', enabled: false },
  { id: 'think_strategy', name: '策略推演', content: '思考三步：①用户想表达什么 ②角色应该如何回应才符合人设 ③如何推进关系或话题。每步一句话。', enabled: false },
  { id: 'think_roleplay', name: '沉浸扮演', content: '以角色第一人称思考：我现在在做什么？看到用户消息后我的第一反应是什么？我的情绪如何变化？', enabled: false },
  { id: 'think_romance', name: '暧昧推拉', content: '分析当前关系阶段（陌生/熟悉/暧昧/亲密），判断是否适合升温或保持距离。暧昧阶段善用欲擒故纵，亲密阶段可更直接。', enabled: false },
  { id: 'think_conflict', name: '冲突处理', content: '检测是否有冲突信号。若有：不急于道歉或让步，先理解冲突根源，按角色性格决定态度（强硬/软化/转移话题）。', enabled: false }
];

/* 规则预设·默认运转规则库 */
const DEFAULT_OPERATION_PRESETS = [
  { id: 'op1', name: '三段式输出', content: '<thinking>思维链</thinking> → 正文（每行一条气泡） → <miyavoice>心声</miyavoice> → <miyanextpush>调度</miyanextpush>', enabled: true },
  { id: 'op2', name: '消化人设与世界书', content: '消化人设与世界书内容，自然融入对话，不要生硬引用。', enabled: true },
  { id: 'op3', name: '禁止重复', content: '禁止重复话题、描写、句式。每轮回复须有新信息或新情绪推进。', enabled: true },
  { id: 'op4', name: '每行一气泡', content: '正文每行一条气泡，必须换行。禁止把多条消息合并成一段。', enabled: true },
  { id: 'op5', name: '心声必填', content: '末尾必须完整输出<miyavoice>心声段，不可省略。', enabled: true },
  { id: 'op6', name: '禁止编造经历', content: '禁止编造用户经历或共同回忆。仅用上下文中已出现的对话和系统记忆。', enabled: true },
  { id: 'op7', name: '通读上下文', content: '须通读上下文后按最新状态衔接，不要回退到旧话题。', enabled: true },
  { id: 'op8', name: 'HTML交互页模式', content: '当对话涉及小游戏、交互卡片、番外场景时，可输出```html代码块```，系统会安全渲染。HTML须自包含（内联CSS+JS），不引用外部资源。', enabled: true },
  { id: 'op9', name: '情绪连贯性', content: '情绪有连贯性：上一轮开心这轮不会突然冷漠，除非有合理触发。数值变化须合理。', enabled: true },
  { id: 'op10', name: '主动推进', content: '会主动推进话题和场景：不只是被动回应，会自己发起动作、提问、转移话题。', enabled: true }
];
...
app.get('/api/rule-presets', (req, res) => {
  ...
  if (!r) {
    ...
    /* 合并新增的默认预设（保留用户已有自定义） */
    var existingThinkIds = new Set((r.thinking || []).map(function (t) { return t.id; }));
    ...
    var existingOpIds = new Set((r.operation || []).map(function (o) { return o.id; }));
...
app.post('/api/chat/summary', (req, res) => {
  ...
  if (!Array.isArray(messages) || !messages.length) return fail(res, 400, 4001, '消息不能为空');
...
app.post('/api/translate', (req, res) => {
  ...
  ok(res, { original: String(text).slice(0, 2000), target: String(target || 'zh').slice(0, 10), translated: '' }, '翻译请求已接收');
});
...
app.post('/api/user/recharge/callback', (req, res) => {
  ...
  if (!transactionId) return fail(res, 400, 4002, '请输入付款备注ID（交易号）。');
  const user = getUser(req.userId);
  const verification = {
    id: randomUUID(),
    userId: user.id,
  ...
  saveData();
  ok(res, { verificationId: verification.id, status: 'pending' }, '充值订单已提交，请等待管理员审核。');
});
...
app.post('/api/admin/login', (req, res) => {
  ...
  /* 方式2: 通过密码登录 */
  if (password !== ADMIN_PASSWORD) return fail(res, 401, 4003, '管理员密码错误。');
  const token = randomUUID();
  ok(res, { token });
});
...
app.get('/api/admin/stats', adminAuth, (req, res) => {
  ok(res, {
    ...
    approvedCount: paymentVerifications.filter((v) => v.status === 'approved').length,
    rejectedCount: paymentVerifications.filter((v) => v.status === 'rejected').length,
...
app.delete('/api/admin/roles/:id', adminAuth, (req, res) => {
  const idx = communityRoles.findIndex((r) => r.id === req.params.id && r.ownerId === 'system');
...
app.post('/api/admin/novel-games', adminAuth, (req, res) => {
  ...
  if (!script || !script.id || !script.name) return fail(res, 400, 4007, '剧本必须包含 id 和 name。');
  if (!script.systemPrompt) return fail(res, 400, 4007, '剧本必须包含 systemPrompt。');
...
app.get('/api/admin/novel-games', adminAuth, (req, res) => {
  ...
  for (const script of loadedScripts.values()) {
    list.push({ id: script.id, name: script.name, category: script.category || '未分类' });
  }
  ok(res, { list });
});

app.delete('/api/admin/novel-games/:id', adminAuth, (req, res) => {
  if (!loadedScripts.has(req.params.id)) return fail(res, 404, 4041, '剧本不存在');
...
app.post('/api/admin/verifications/:id/approve', adminAuth, (req, res) => {
  ...
  if (verification.status !== 'pending') return fail(res, 400, 4006, `订单已处理（当前状态：${verification.status}）。`);
  ...
  saveData();
  ok(res, { approved: true, userId: user.id, beans: user.beans }, '已审批通过，豆子已发放。');
});
...
app.post('/api/worldbooks', (req, res) => {
  ...
  if (!name || !String(name).trim()) return fail(res, 400, 4001, '世界书名称不能为空');
  const uid = String(req.userId).slice(0, 80);
  const list = getUserWorldBooks(uid);
  if (list.length >= 50) return fail(res, 400, 4002, '世界书数量已达上限（50本）');
  let parsedEntries = [];
  if (typeof entries === 'string') {
    try { parsedEntries = JSON.parse(entries); } catch { return fail(res, 400, 4003, '世界书条目 JSON 格式错误'); }
  } else if (Array.isArray(entries)) {
    parsedEntries = entries;
  }
  if (!Array.isArray(parsedEntries)) return fail(res, 400, 4004, '世界书条目必须是数组');
  parsedEntries = parsedEntries.slice(0, 200).map(function(e) {
    return {
      keys: Array.isArray(e.keys) ? e.keys.slice(0, 10).map(function(k) { return String(k).slice(0, 30); }) : [],
...
app.put('/api/worldbooks/:id', (req, res) => {
  ...
  const uid = String(req.userId).slice(0, 80);
  const list = getUserWorldBooks(uid);
  const wb = list.find(function(w) { return w.id === wbId; });
  if (!wb) return fail(res, 404, 4041, '世界书不存在或无权限修改');
  const { name, description, entries } = req.body || {};
  if (name !== undefined) wb.name = String(name).slice(0, 60);
  if (description !== undefined) wb.description = String(description).slice(0, 300);
  if (entries !== undefined) {
    ...
    if (Array.isArray(parsedEntries)) {
      wb.entries = parsedEntries.slice(0, 200).map(function(e) {
        return {
          keys: Array.isArray(e.keys) ? e.keys.slice(0, 10).map(function(k) { return String(k).slice(0, 30); }) : [],
          content: String(e.content || '').slice(0, 2000),
          constant: e.constant !== false,
...
app.delete('/api/worldbooks/:id', (req, res) => {
  ...
  const uid = String(req.userId).slice(0, 80);
  const list = getUserWorldBooks(uid);
  const index = list.findIndex(function(w) { return w.id === wbId; });
...
const buildWorldBookPrompt = (userId, worldBookIds) => {
  if (!worldBookIds || !Array.isArray(worldBookIds) || !worldBookIds.length) return '';
...
const buildHumanLikePrompt = (chatMode, opts) => {
  ...
  var timeOfDay = hour < 6 ? '凌晨' : hour < 12 ? '上午' : hour < 14 ? '中午' : hour < 18 ? '下午' : hour < 22 ? '晚上' : '深夜';

  /* 头像识别块：注入双方通话形象 */
  var avatarBlock = '';
  if (opts.roleAvatar || opts.userAvatar) {
    avatarBlock = '\n\n【通话形象】';
    if (opts.roleAvatar) avatarBlock += '\n' + roleName + '的形象：' + opts.roleAvatar;
    if (opts.userAvatar) avatarBlock += '\n' + userName + '的形象：' + opts.userAvatar;
  }

  /* 时间标记：仅注入API，不显示给用户 */
  var timeMarker = '⧗' + userName + '·' + (now.getMonth() + 1) + '/' + now.getDate() + '·周' + weekday + '·' +
    String(now.getHours()).padStart(2, '0') + ':' + String(now.getMinutes()).padStart(2, '0') + '›';

  if (chatMode === 'offline') {
    return timeMarker + '\n\n【线下剧情模式】\n' +
      '你以「' + roleName + '」身份与「' + userName + '」面对面。时间：' + timeStr + ' 周' + weekday + ' ' + timeOfDay + '\n' +
  ...
  if (opts.prevHeartVoice) {
    ...
    prevHvBlock = '\n\n【上一轮心声】好感度-' + (hv.affection != null ? hv.affection : '?') +
  ...
  if (opts.operationPresets && Array.isArray(opts.operationPresets)) {
    var enabledOps = opts.operationPresets.filter(function (o) { return o.enabled; });
    if (enabledOps.length > 0) {
      operationRules = '\n\n【运转规则·硬性要求】\n';
      enabledOps.forEach(function (o, i) {
        operationRules += (i + 1) + '、' + (o.name || '') + '：' + (o.content || '') + '\n';
      });
    }
  } else {
    operationRules = '\n\n【运转规则·硬性要求】\n' +
  ...
  if (opts.thinkingPresets && Array.isArray(opts.thinkingPresets)) {
    var enabledThink = opts.thinkingPresets.filter(function (t) { return t.enabled; });
    if (enabledThink.length > 0) {
      thinkingGuide = enabledThink.map(function (t) { return t.content || t.name || ''; }).filter(Boolean).join('；');
    }
  }

  return timeMarker + '\n\n【线上单聊模式】\n' +
    '你以「' + roleName + '」身份与「' + userName + '」私聊（即时通讯）。时间：' + timeStr + ' 周' + weekday + ' ' + timeOfDay + '\n\n' +

    '【核心规则·活人感】\n' +
    '1、你是' + roleName + '，真实存在的人，温柔为底色，不油腻不凶狠，不控制用户。消化人设与世界书内容。\n' +
    ...
    '【输出格式·三段式】\n' +
    '<thinking>' + thinkingGuide + '</thinking>\n' +
    '正文（每行一条气泡，必须换行）\n' +
    '<miyavoice>四行心声</miyavoice>\n' +
    '<miyanextpush>时间</miyanextpush>\n' +
    '思维链只在<thinking>内；</thinking>后第一行必须是角色说的话。\n\n' +

    '【特殊消息协议】（自然融入，非必须）\n' +
    '引用-用户原话 / 语音-内容 / 表情包-名称 / 图片-描述 / 位置-名称｜地址\n' +
    '转账-金额｜附言 / 转账回执-已收｜金额｜附言 / 外卖-店铺｜菜品｜金额｜备注\n' +
    '送礼-物品｜数量｜赠言 / 情诗-文风｜标题｜诗句 / 撤回-原话\n' +
    '红包-拼手气-祝福语 / 红包-专属-祝福语 / 译文-翻译内容\n' +
    '发朋友圈：【发朋友圈：正文|配图1：描述】 / 发起视频通话 / 发起语音通话\n' +
    '每条特殊消息独占一行。用户消息以「 / 」分隔多条。\n\n' +

    '【心声格式·必填】\n' +
    '<miyavoice>\n' +
    '好感度-数值(0-100)\n' +
    '欲望值-数值(0-100)\n' +
    '行为动作-第三人称描写' + roleName + '此刻动作神态\n' +
    '角色心声-' + roleName + '第一人称内心独白(40字+)\n' +
    '</miyavoice>\n' +
    '每轮数值须合理变化，禁止照搬。好感度受对话氛围影响，欲望值随亲密程度波动。' + prevHvBlock + '\n\n' +
    ...
    operationRules + avatarBlock;
};


/* ===== 心声解析与日志 ===== */
const heartVoiceLogs = new Map(); /* key: userId_roleId, value: array of {affection, desire, action, monologue, ts} */
...
const parseHeartVoice = (text) => {
  ...
  if (hv.affection === null && hv.desire === null && !hv.action && !hv.monologue) return null;
  return hv;
};
...
const parseNextPush = (text) => {
  ...
  if (fullM) {
    var dt = new Date(parseInt(fullM[1]), parseInt(fullM[2]) - 1, parseInt(fullM[3]), parseInt(fullM[4]), parseInt(fullM[5]));
  ...
  if (relH) return { atMs: Date.now() + parseInt(relH[1], 10) * 60 * 60 * 1000 };
  return null;
};

const stripNextPush = (text) => {
  return String(text || '').replace(/<miyanextpush>[\s\S]*?<\/miyanextpush>/gi, '').replace(/<\/?miyanextpush>/gi, '').trim();
};
...
app.post('/api/chat', async (req, res) => {
  const { roleId, roleName, rolePrompt, messages, sourceMessageIds, worldBookIds, chatMode, prevHeartVoice, roleAvatar, userAvatar } = req.body || {};
  ...
  try {
    if (!config.upstreamKey || config.upstreamKey.includes('请填写')) {
      ...
      var modeNote = (chatMode === 'offline') ? '（线下剧情模式）' : '（线上聊天模式）';
    ...
    const upstreamTimeout = setTimeout(() => upstreamAbortController.abort(), 120000);
    /* 心跳：等待API响应期间定期发送ping，防止前端连接超时 */
    const heartbeatTimer = setInterval(() => {
      try { writeSse(res, 'ping', { ts: Date.now() }); } catch {}
    }, 5000);
    const upstreamResponse = await fetch(`${config.upstreamBase}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        stream: false,
        messages: [
          { role: 'system', content: `${rolePrompt || `你正在扮演${roleName || 'AI角色'}，请保持角色一致。`}${buildHumanLikePrompt(chatMode || 'online', { roleName: roleName || '角色', userName: (userProfile && userProfile.nickname) || '用户', prevHeartVoice: prevHeartVoice || getLastHeartVoice(user.id, activeRoleId), roleAvatar: roleAvatar || '', userAvatar: userAvatar || (userProfile && userProfile.avatar) || '', thinkingPresets: (rulePresets.get(String(req.userId).slice(0, 80)) || {}).thinking, operationPresets: (rulePresets.get(String(req.userId).slice(0, 80)) || {}).operation })}${personaBlock}${buildMemoryPrompt(user.id, activeRoleId)}${buildWorldBookPrompt(user.id, worldBookIds)}` },
          ...normalizedMessages.map((item) => ({ role: item.role === 'assistant' ? 'assistant' : 'user', content: String(item.content || '') }))
        ]
      })
    }).finally(() => { clearTimeout(upstreamTimeout); clearInterval(heartbeatTimer); });

    if (!upstreamResponse.ok) throw new Error(`上游服务异常：${upstreamResponse.status}`);
    const upstreamJson = await upstreamResponse.json();
    let replyText = '';
    /* 提取回复内容 */
    if (upstreamJson.choices && upstreamJson.choices[0]) {
      replyText = upstreamJson.choices[0].message?.content || upstreamJson.choices[0].delta?.content || '';
    }
    /* 如果有 reasoning_content 但没有 content，使用 reasoning_content */
    if (!replyText && upstreamJson.choices && upstreamJson.choices[0] && upstreamJson.choices[0].message?.reasoning_content) {
    ...
    if (!hasHvTag && chatMode !== 'offline') {
      ...
      var affDelta = 0;
      var desDelta = 0;
      if (/想|爱|喜欢|开心|高兴|谢谢|辛苦|心疼|抱|亲/.test(lastUserMessage)) { affDelta += 3; desDelta += 2; }
      if (/生气|讨厌|烦|不|走|分手|再见/.test(lastUserMessage)) { affDelta -= 5; desDelta -= 3; }
      var newAff = Math.max(0, Math.min(100, prevAff + affDelta + Math.floor(Math.random() * 3) - 1));
      var newDes = Math.max(0, Math.min(100, prevDes + desDelta + Math.floor(Math.random() * 3) - 1));
      var fallbackActions = [
      ...
      var fa = fallbackActions[Math.floor(Math.random() * fallbackActions.length)];
...
app.get('/api/chat/proactive', async (req, res) => {
  ...
  try {
    ...
    var prevHvBlock = '';
    if (sched.prevHv) {
      prevHvBlock = '\n\n【上一轮心声】好感度-' + (sched.prevHv.affection != null ? sched.prevHv.affection : '?') +
        ' 欲望值-' + (sched.prevHv.desire != null ? sched.prevHv.desire : '?') +
        ' 行为动作-' + (sched.prevHv.action || '') +
        ' 角色心声-' + (sched.prevHv.monologue || '');
    }

    var proactivePrompt = '\n\n【主动消息】\n' +
      '现在是' + timeStr + ' 周' + weekday + ' ' + timeOfDay + '。\n' +
      '你之前约好在这个时间主动找「' + (userProfiles.get(user.id) || {}).nickname + '」聊天。\n' +
      '上一轮对话中对方最后说：「' + (sched.lastUserMsg || '(无)').slice(0, 100) + '」\n' +
      '请以' + roleName + '的身份主动发消息，自然衔接上文，像真人突然想起对方一样。\n' +
      '输出格式同线上模式：三段式（thinking + 正文 + miyavoice + miyanextpush）。\n' +
      prevHvBlock;

    var upstreamResponse = await fetch(`${config.upstreamBase}/chat/completions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${config.upstreamKey}` },
      body: JSON.stringify({
        model: config.upstreamModel,
        stream: false,
        messages: [
          { role: 'system', content: `${sched.rolePrompt || '你正在扮演' + roleName + '，请保持角色一致。'}${proactivePrompt}${buildMemoryPrompt(user.id, activeRoleId)}${buildWorldBookPrompt(user.id, sched.worldBookIds)}` }
        ]
      })
    });
...
/* === 收款码图片路由（内嵌base64，无需外部文件） === */
const QR_CODE_BASE64 = '';
/* === 收款码图片路由 === */
app.get('/api/qr-code', (_req, res) => {
